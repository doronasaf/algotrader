            ...
            switch (phase) {
                case "A": // Accumulation
                    if (!support || !resistance) {
                        ...
                        const accumulationAchieved = await bulishMomentum.evaluateAccumulation();
                        if (accumulationAchieved) {
                            // console.log(`Ticker: ${ticker} | Momentum confirmed. Stabilizing range...`);
                            // console.log(`Ticker: ${ticker} | Phase: ${phase} | Close: ${close}, High: ${high}, Low: ${low}, Volume: ${volume}, DynamicVolumeThreshold: ${dynamicVolumeThreshold}`);
                            logger.info(`Ticker ${ticker} | Momentum confirmed. Stabilizing range...`);
                            phase = "B";
                        }
                    }
                    break;
                ...
                case "B": // Breakout
                    const breakoutConfirmed = await bulishMomentum.evaluateBreakout();
